...
指定できる範囲は1~384です。
デフォルトは4です。
384より大きい数字や、384分音符の整数倍の音長にならない数字を指定した場合、NAMIDIではエラーにします。
指定可能な値は以下になります。
```
...
### @bs
チャネルの音色変更で使用します。
MIDIメッセージのバンクセレクトMSB+LSBに相当します。
MSBとLSBをカンマ区切りで指定します。
```
@bs 0, 120 // MSB=0, LSB=120 
```
...
```
// GeneralUser GS の Standard Drums
@bs 0, 120 // MSB=0, LSB=120
@pc 0      // ProgramNo=0

// SGM-V2.01 の Standard Drums
@bs 0, 127 // MSB=0, LSB=127
@pc 0      // ProgramNo=0
```
...
SMFファイルに出力した際、コントロールチェンジ(Cn 0Ah) パンとして出力されます。
